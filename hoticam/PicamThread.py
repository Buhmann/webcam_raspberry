...
class PicamThread(threading.Thread):
    ...
    def __init__(self):
        super(PicamThread, self).__init__()
    ...
    def run(self): 
        ...
        # Get first image
        image1, buffer1 = self.captureTestImage(cameraSettings, testWidth, testHeight)
        # Reset last capture time
        lastCapture = time.time()
      
        while (not self._stop.isSet()):
            # Get comparison image
            image2, buffer2 = self.captureTestImage(cameraSettings, testWidth, testHeight)
            ...
            if takePicture:
                lastCapture = time.time()
                self.saveImage(cameraSettings, saveWidth, saveHeight, saveQuality, diskSpaceToReserve)
